...
public class CnSControlAlgorithmPanel extends CnSPanel implements ChangeListener {
	...
	protected void initGraphics() {
		...
		addComponent(FTButton, 0, 0, 1, 1, 1.0, 0.0, NORTHWEST, NONE, 5, 5, 5, 5, 0, 0);
		TFitButton = new JRadioButton("TFit");
		addComponent(TFitButton, 0, 1, 1, 1, 1.0, 0.0, NORTHWEST, NONE, 0, 5, 5, 5, 0, 0);		
		OCGButton = new JRadioButton("OCG");
		addComponent(OCGButton, 0, 3, 1, 1, 1.0, 0.0, NORTHWEST, NONE, 0, 5, 5, 5, 0, 0);		
	...
	private void initListeners() {
		...
		TFitButton.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				if (((JRadioButton)e.getSource()).isSelected()) {
					addComponent(tfitOptionsPanel, 0, 2, 1, 1, 1.0, 1.0, NORTHWEST, HORIZONTAL, 0, 20, 20, 5, 0, 0);
					CnSEvent ev = new CnSEvent(CnSAlgorithmManager.SET_SELECTED_ALGORITHM, CnSEventManager.ALGORITHM_MANAGER);
		...
		OCGButton.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				if (((JRadioButton)e.getSource()).isSelected()) {
					addComponent(ocgOptionsPanel, 0, 4, 1, 1, 1.0, 1.0, NORTHWEST, HORIZONTAL, 0, 20, 20, 5, 0, 0);
					CnSEvent ev = new CnSEvent(CnSAlgorithmManager.SET_SELECTED_ALGORITHM, CnSEventManager.ALGORITHM_MANAGER);

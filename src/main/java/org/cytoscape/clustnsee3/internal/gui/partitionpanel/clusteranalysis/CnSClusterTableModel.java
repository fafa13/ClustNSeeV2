...
public class CnSClusterTableModel extends CnSTableModel {
	...
	public void init() {
		...
		for (CnSNodeAnnotation a : annotations) {
			...
			n = ((Vector<?>)(CnSEventManager.handleMessage(ev, true)).getValue()).size();
		...
		switch(column) {
			...
			case 2 :	if (network == null) return 0;
						...
						if (selectedCluster == null)
							...
							node_count = ((Vector<?>)CnSEventManager.handleMessage(ev, false).getValue()).size();
						}
			...
			case 3 :	if (selectedCluster == null) return "N/A";
 						...
 						CnSNodeAnnotation annot = (CnSNodeAnnotation)CnSEventManager.handleMessage(ev, false).getValue();
			...
			case 4 :	if (selectedCluster == null) return "N/A";
			 			...
			 			ev = new CnSEvent(CnSNodeAnnotationManager.GET_ANNOTATION, CnSEventManager.ANNOTATION_MANAGER, this.getClass());
			 			ev.addParameter(CnSNodeAnnotationManager.ANNOTATION, o);
			 			annot = (CnSNodeAnnotation)CnSEventManager.handleMessage(ev, false).getValue();
